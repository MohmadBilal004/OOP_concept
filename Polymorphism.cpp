 ...
 class Employee: AbstractEmployee    // ssign the contract //
{
 ...
 protected:   //protected is where only the derived class of this parent class can access//
  ...
  void AskForPromotion()  // implementation of the contract //
  {
    ...
    virtual void work()  // here virtual is like where it checks whether if the child class also has the same function then execute the child function//
    {                    // else it will display the function in parent class//
        cout<<name<<" checks emails, proficiency in MS Office package... "<<endl;
    }
};
...
class Developer: public Employee //This inheritence become private in default so we have to add public if have to add oyside trhe class//
{
    ...
    void work()
    {
        cout<<name<<" does programming in "<<language<<" language."<<endl;
    }
};
...
int main()
{
      /* Polymorphism is when parent class referrence is used for child class object */
